...
namespace LiveAthan.Helpers
{
    ...
    */
    ...
    public class PrayerTimes
    {
        ...
        //---------------------- Global Variables --------------------
        private int calcMethod = (int)CalculationMethod.ISNA;		// caculation method
        private int asrJuristic = (int)JuristicMethod.Hanafi;		// Juristic method for Asr
        ...
        public PrayerTimes()
        {
            ...
            this.methodParams[(int)CalculationMethod.Jafari] = new double[] { 16, 0, 4, 0, 14 };
            this.methodParams[(int)CalculationMethod.Karachi] = new double[] { 18, 1, 0, 0, 18 };
            this.methodParams[(int)CalculationMethod.ISNA] = new double[] { 15, 1, 0, 0, 15 };
            this.methodParams[(int)CalculationMethod.MWL] = new double[] { 18, 1, 0, 0, 17 };
        ...
        public Times getPrayerTimes(int year, int month, int day, double latitude, double longitude, int timeZone)
        {
        ...
        // return prayer times for a given date
        public Times getDatePrayerTimes(int year, int month, int day, double latitude, double longitude, int timeZone)
        {
            this.lat = latitude;
            this.lng = longitude;
        ...
        public Times makeTimes(double[] times)
        {
            return new Times
            {
                Fajr = this.floatToTimeSpan(times[0]),
                Sunrise = this.floatToTimeSpan(times[1]),
                Dhuhr = this.floatToTimeSpan(times[2]),
                Asr = this.floatToTimeSpan(times[3]),
                Sunset = this.floatToTimeSpan(times[4]),
                Maghrib = this.floatToTimeSpan(times[5]),
                Isha = this.floatToTimeSpan(times[6]),
            };
        }
        ...
        {
            return false; // I think we have dst-adjusted client time zone, so we don't need this.
            // TODO: Uses client DST?
